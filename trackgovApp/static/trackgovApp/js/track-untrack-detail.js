    ...
    for (var i=0; i<check_box_track.length; i++) {
        ...
        check_box_track[i].onclick = function(e) {
            if (e.target.checked == true) {
                ...
                xhttp.open("GET", "http://127.0.0.1:8000/trackbilldetailpreferences/?bill_id=" + e.target.id.substring(6) + "&option=" + track_label[e.target.index].dataset.onLabel, true);
                xhttp.send();
                ...
                xhttp.open("GET", "http://127.0.0.1:8000/trackbilldetailpreferences/?bill_id=" + e.target.id.substring(6) + "&option=" + track_label[e.target.index].dataset.offLabel, true);
                xhttp.send();
